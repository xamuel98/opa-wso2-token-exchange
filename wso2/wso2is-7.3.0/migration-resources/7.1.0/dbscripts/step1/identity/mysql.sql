-- Disable foreign key checks for the duration of DDL operations to avoid issues
-- if tables are being dropped/recreated out of order, or during column modifications.
...
-- Column Additions - Wrapped in stored procedures for idempotence
...
-- INDEX CREATION - Make idempotent using stored procedures to create ONLY IF NOT EXISTS

-- Helper procedure to create an index if it does not exist
